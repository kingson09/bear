...
/**
 * Created by bjliuzhanyong on 2018/3/15.
 */
...
public class SnappyInputStream2 extends InputStream {
  ...
  private static boolean isValidHeader(byte[] header) throws IOException {
    ...
    if (codec.isValidMagicHeader()) {
      ...
      if (codec.version < SnappyCodec.MINIMUM_COMPATIBLE_VERSION) {
        throw new SnappyIOException(SnappyErrorCode.INCOMPATIBLE_VERSION, String
            .format("Compressed with an incompatible codec version %d. At least version %d is required", codec.version,
                SnappyCodec.MINIMUM_COMPATIBLE_VERSION));
      }
      return true;
    } else {
      return false;
  ...
  protected boolean hasNextChunk() throws IOException {
    ...
    int chunkSize = MappedSnappyOutputStream.readInt(header, 0);
    if (chunkSize == SnappyCodec.MAGIC_HEADER_HEAD) {
      ...
      if (readBytes < remainingHeaderSize) {
        throw new SnappyIOException(SnappyErrorCode.FAILED_TO_UNCOMPRESS,
    ...
    if (uncompressedLength != actualUncompressedLength) {
      throw new SnappyIOException(SnappyErrorCode.INVALID_CHUNK_SIZE, String
          .format("expected %,d bytes, but decompressed chunk has %,d bytes", uncompressedLength,
