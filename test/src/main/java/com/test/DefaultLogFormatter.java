...
public class DefaultLogFormatter implements Formatter<LogEvent, byte[]> {
  ...
  public byte[] formatJson(LogEvent event) {
    ...
    try {
      ...
      if (!TextUtils.isEmpty(location)) {
        jo.put(TAG_LOCATION, location);
      }
      if (!TextUtils.isEmpty(log_time)) {
        jo.put(TAG_LOG_TIME, log_time);
      }
      if (!TextUtils.isEmpty(event.getMessage())) {
        jo.put(TAG_MESSAGE, "[" + event.getThreadName() + "] " + event.getMessage());
      }
      if (!TextUtils.isEmpty(event.getTag())) {
        jo.put(TAG_LOG_TAG, event.getTag());
      }
      jo.put(TAG_LEVEL, event.getLevel().toString());

      byte[] result = (jo.toString() + lineSep).getBytes(charset);
      clock.start();
      return result;
  ...
  public byte[] formatGson(LogEvent event) {
    ...
    if (!TextUtils.isEmpty(location)) {
      jo.addProperty(TAG_LOCATION, location);
    }
    if (!TextUtils.isEmpty(log_time)) {
      jo.addProperty(TAG_LOG_TIME, log_time);
    }
    if (!TextUtils.isEmpty(event.getMessage())) {
      jo.addProperty(TAG_MESSAGE, "[" + event.getThreadName() + "] " + event.getMessage());
    }
    if (!TextUtils.isEmpty(event.getTag())) {
      jo.addProperty(TAG_LOG_TAG, event.getTag());
    }
    jo.addProperty(TAG_LEVEL, event.getLevel().toString());
